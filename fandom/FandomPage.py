...
from fandom.error import (ODD_ERROR_MESSAGE, FandomError, HTTPTimeoutError,
                          PageError, RedirectError)

from .util import _wiki_request, stdout_encode
...
class FandomPage(object):
    ...
    def __init__(
        self, wiki, language, title=None, pageid=None, redirect=True, preload=False
    ):
    ...
    def __repr__(self):
        return stdout_encode("<FandomPage '{}'>".format(self.title))
    ...
    def __load(self, redirect=True, preload=False):
        ...
        if (not redirect) and ("redirects" in query):
            ...
            raise PageError(
                self.pageid if self.pageid else None, self.title if self.title else None
            )
        else:
    ...
    def content(self):
        ...
        def clean(content):
            ...
            for key in keys:
                if content[key] != "":
                    content[key] = re.sub("\xa0", " ", content[key])
        ...
        if not getattr(self, "_content", False):
            ...
            while isinstance(next_node, NavigableString) or next_node.name in [
                "div",
                "figure",
                "table",
            ]:
            ...
            while True:
                if next_node is None:
                    ...
                    if (
                        next_node.name
                        and next_node.name.startswith("h")
                        and next_node.name[1:].isdigit()
                    ):
                        level_tree[-1]["content"] = section_text
                    ...
                    elif (not next_node.has_attr("class")) or (
                        next_node["class"] and next_node["class"][0] != "printfooter"
                    ):
                        section_text += "\n" + next_node.get_text()
                next_node = next_node.nextSibling
    ...
    def revision_id(self):
        ...
        if not getattr(self, "_revid", False):
            ...
            request = _wiki_request(query_params)
            self._revision_id = request["query"]["pages"][str(self.pageid)][
                "revisions"
            ][0]["revid"]
    ...
    def images(self):
        ...
        if not getattr(self, "_images", False):
            ...
            if "images" in request["query"]["pages"][str(self.pageid)]:
                images = [
                    image["title"]
                    for image in request["query"]["pages"][str(self.pageid)]["images"]
                ]
            else:
                images = []
    ...
    def section(self, section_title: str):
        ...
        if section_title.lower() == self.title.lower():
            ...
            section_title.lower() not in [i.lower() for i in self.sections]
            or "sections" not in self.content
        ):
            return None
        else:
            return get_section_recursive(
                self.content["sections"], section_title.lower()
            )
